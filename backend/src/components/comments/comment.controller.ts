...
export class CommentController implements Controller {
    ...
    public initRoutes() {
        ...
        this.router.get(
            `${this.path}/amount/:id`,
            this.getAmountOfCommentsInPost
        );
    }
    ...
    private deleteComment = async (
        ...
        try {
            ...
            if (token) {
                ...
                const user = await this.user.findById(decodedToken._id);
                if (user) {
                    const comment = await this.comment.findById(
                        request.params.id
                    );
                    if (comment) {
                        if (comment.byUser === user._id) {
                            await this.comment.findByIdAndRemove(
                                request.params.id
                            );
    ...
    private getAmountOfCommentsInPost = async (
        request: Request,
        response: Response,
        next: NextFunction
    ): Promise<void> => {
        try {
            await this.comment.countDocuments(
                { toPost: request.params.id },
