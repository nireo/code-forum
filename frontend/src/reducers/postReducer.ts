...
const classPostService = new PostService();
...
const reducer = (state: PostInterface[] = [], action: any) => {
    switch (action.type) {
        ...
        case "NEW_COMMENT":
            ...
            if (postToComment) {
                postToComment.comments = postToComment.comments.concat(
                    action.data
                );
            }
        ...
        case "UPDATE_POST":
            return state.map(p => (p._id === action._id ? action.data : p));
        default:
            return state;
    }
};
...
export const updatePost = (post: PostInterface, id: string) => {
    return async (dispatch: Dispatch) => {
        const newPost = await classPostService.updatePost(post, id);
        dispatch({
            type: "UPDATE_POST",
            data: newPost,
            id: id
